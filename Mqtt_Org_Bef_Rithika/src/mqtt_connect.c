...
void mqtt_send_file(mqtt_conn_t *mqtt_cfg, const char *filename)
{
    ...
    while ((bytes_read = fread(buffer, 1, PAYLOAD_BUFFER_SIZE, fp)) > 0)
    {
        ...
        int rc = MQTTAsync_sendMessage(mqtt_cfg->client,
                                       mqtt_cfg->cfg.cyclic_dlms_data_topic,
                                       &msg,
                                       &opts);
...
void mqtt_send_msg(mqtt_conn_t *mqtt_cfg, const char *mqtt_msg, int msg_size, int topic_type)
{
    ...
    if(topic_type == MODBUS_DATA_TOPIC)
    {
        rc = MQTTAsync_sendMessage(mqtt_cfg->client,
                                   mqtt_cfg->cfg.cyclic_modbus_data_topic,
                                   &msg,
                                   &opts);
    }
    ...
    {
        rc = MQTTAsync_sendMessage(mqtt_cfg->client,
                                   mqtt_cfg->cfg.health_check_data_topic,
                                   &msg,
                                   &opts);
    }
    else
    {
        rc = MQTTAsync_sendMessage(mqtt_cfg->client,
                                   mqtt_cfg->cfg.cmd_response_topic,
                                   &msg,
                                   &opts);
    }
...
void mqtt_subscribe_topic(mqtt_conn_t *mqtt_cfg)
{
    ...
    {
        int rc = MQTTAsync_subscribe(mqtt_cfg->client, mqtt_cfg->cfg.subscribe_topics[i], mqtt_cfg->cfg.qos, &opts);
        if (rc != MQTTASYNC_SUCCESS)
        {
            printf("Subscribe Topics Failed !!!\n");
            LOG_ERROR(stderr, "Failed to subscribe the topic %s", mqtt_cfg->cfg.subscribe_topics[i]);
            return;
        }
        printf("Topics SUbscribed Successfully !!!\n");
        LOG_INFO("%s\n", mqtt_cfg->cfg.subscribe_topics[i]);
    }
...
int processServerMsg(mqtt_conn_t *conn, const char *msg)
{
    ...
    if (strcmp(cmd.type, "GetDay") == 0 && cmd.args[1][0] != '\0')
    {
        ...
        if (res.status == 0)
        {
            ...
            mqtt_send_file(current_active, res.filename);
...
int update_mqtt_status(char *status)
{
    ...
    printf("1111111111111111111111111 current active primary  %d current activesecondary %d \n", current_active_primary, current_active_secondary);
