...
typedef struct
{
    ...
    char dcu_location[METER_FIELD_LEN];
    char modem_imei[METER_FIELD_LEN];
...
static int fetch_dcu_info(redisContext *ctx, dcu_info_t *info)
{
    ...
    redis_hget(ctx, HASH_GENERAL_CDF, "attribute4", info->attr4, sizeof(info->attr4));
    redis_hget(ctx, HASH_GENERAL_CDF, "attribute5", info->attr5, sizeof(info->attr5));
...
static int fetch_dcu_status(redisContext *ctx, dcu_status_t *st)
{
    ...
    redis_hget(ctx, NTP_CFG, "ntp_sync_time", st->time_synched, sizeof(st->time_synched));

    char redis_uptime[64];
    /* ACTIVE_SINCE from dcu_uptime */
    redis_hget(ctx, HASH_DCU_INFO, "dcu_uptime", redis_uptime, sizeof(redis_uptime));

    long int uptime = 0;
    uptime = atoi(redis_uptime);
    ...
    snprintf(st->active_since, sizeof(st->active_since), "%s", uptime_str);
    /* Link status */
    redis_hget(ctx, HASH_MODEM_STATUS, "ppp_connection_status", ppp_status, sizeof(ppp_status));
    if (strcmp(ppp_status, "connected") == 0)
    {
                                 ...
                                 uint32_t *count)
{
    ...
    for (; i < num_meters && *count < MAX_METERS; i++)
    {
        ...
        {
            char meter_enbl[METER_FIELD_LEN] = {0};
            snprintf(field, sizeof(field), "enable_meter[%u]", i);
            redis_hget(ctx, hash_name, field, meter_enbl, sizeof(meter_enbl));

            if (strcmp(meter_enbl, "0") == 0)
            {
                snprintf(m->comm_inhibit, sizeof(m->comm_inhibit), "YES");
            }
            else
            {
                               ...
                               uint32_t count)
{
    ...
    for (i = 0; i + 1 < (uint32_t)reply->elements; i += 2)
    {
        ...
        for (j = 0; j <= count; j++)
        {
            if (j == meter_id && !strcmp(port_buf, "2"))
            {
...
static int enrich_meter_status_ser(redisContext *ctx,
                                   meter_entry_t *meters,
                                   uint32_t count, char *port_label)
{
    redisReply *reply = NULL;
    uint32_t i, j;
    ...
    for (i = 0; i + 1 < (uint32_t)reply->elements; i += 2)
    {
        ...
        for (j = 0; j <= count; j++)
        {
            ...
            if (!strcmp(port_label, "serial0"))
            {
                if (j == meter_id && !strcmp(port_buf, "0"))
                {
                    snprintf(meters[j].port, sizeof(meters[j].port), "%s", port_buf);
                    snprintf(meters[j].serial_number, sizeof(meters[j].serial_number), "%s", sn_buf);
                    LOG_DEBUG("enrich_meter_status: matched id=%u port=%s sn=%s",
            ...
            {
                if (j == meter_id && !strcmp(port_buf, "1"))
                {
                    snprintf(meters[j].port, sizeof(meters[j].port), "%s", port_buf);
                    snprintf(meters[j].serial_number, sizeof(meters[j].serial_number), "%s", sn_buf);
                    LOG_DEBUG("enrich_meter_status: matched id=%u port=%s sn=%s",
    ...
    for (i = 0; i <= count; i++)
    {
        ...
        if (redis_hget(ctx, HASH_METER_COMMN_STATUS, commn_field, commn_val, sizeof(commn_val)) == 0)
        {
            if (strcmp(commn_val, "Communicating") == 0)
            {
                ...
                snprintf(meters[i - 1].comm_status, sizeof(meters[i - 1].comm_status), "CONNECTED");
            }

            else
            {
                snprintf(meters[i - 1].comm_status, sizeof(meters[i - 1].comm_status), "NOT CONNECTED");
            }
...
//     enrich_meter_status_ser(ctx, meters, meter_count);
...
// Gokul commented the below function --> 03/07/2026
// int build_health_status_xml(redisContext *ctx, char *out_buf, size_t out_sz, int *output_file_sz)
// {
...
int build_health_status_json(redisContext *ctx,char *out_buf,size_t out_sz,int *output_file_sz)
{
    dcu_info_t dcu_inf;
    dcu_status_t dcu_st;
    char dev_type[8] = {0};
    char ser1_dev_type[8] = {0};
    static meter_entry_t meters[MAX_METERS];
    int i;
    uint32_t meter_count = 0;
    int first_meter = 1;
    ...
    if (fetch_dcu_status(ctx, &dcu_st) < 0)
    {
        LOG_ERROR("Error fetching dcu_status");
        return rc;
    }
    fetch_meters_from_cfg(ctx,HASH_ETH_METER_CFG,"eth",meters,&meter_count);
...
#define APPEND(fmt, ...)                                                     \
    do                                                                       \
    {                                                                        \
        n = snprintf(out_buf + written,                                      \
                     out_sz - written,                                       \
                     fmt,                                                    \
                     ##__VA_ARGS__);                                         \
        if (n < 0 || (size_t)n >= out_sz - written)                          \
        {                                                                    \
            LOG_ERROR("build_health_status_json: output buffer overflow");   \
        }                                                                    \
        written += (size_t)n;                                                \
    } while (0)
    ...
    APPEND("  \"TYPE\": \"CYCLIC_MESSAGE\",\n");
    APPEND("  \"SEQ_NUM\": \"0001\",\n");
    APPEND("  \"DATATYPE\": \"HEALTH_MESSAGE\",\n");

    /**************** NP ****************/

    APPEND("  \"NP\": {\n");
    APPEND("    \"DEVNAME\": \"%s\",\n",dcu_inf.dcu_name);
    APPEND("    \"DEV_LOC\": \"%s\",\n", dcu_inf.dcu_location);
    APPEND("    \"SN\": \"%s\",\n",dcu_inf.serial_num);
    APPEND("    \"IMEI\": \"%s\",\n", dcu_inf.modem_imei);
    APPEND("    \"FW_VER\": \"%s\",\n",dcu_inf.firmware);
    APPEND("    \"TS\": \"%s\"\n",dcu_st.time_now);
    ...
    APPEND("  \"DCU_DETAILS\": {\n");
    APPEND("    \"ATTRIBUTE_1\": \"%s\",\n",dcu_inf.attr1);
    APPEND("    \"ATTRIBUTE_2\": \"%s\",\n",dcu_inf.attr2);
    APPEND("    \"ATTRIBUTE_3\": \"%s\",\n",dcu_inf.attr3);
    APPEND("    \"ATTRIBUTE_4\": \"%s\",\n",dcu_inf.attr4);
    APPEND("    \"ATTRIBUTE_5\": \"%s\"\n",dcu_inf.attr5);
    APPEND("  },\n");
    ...
    APPEND("      \"POWER\": \"%s\",\n",dcu_st.power);
    APPEND("      \"TIME\": \"%s\",\n",dcu_st.time_now);
    APPEND("      \"ACTIVE_SINCE\": \"%s\",\n", dcu_st.active_since);
    APPEND("      \"TIME_SYNCHED\": \"%s\",\n", dcu_st.time_synched);
    APPEND("      \"LINK_STATUS\": {\n");
    APPEND("        \"CONNECTED_ON\": \"%s\",\n",dcu_st.connected_on);
    APPEND("        \"ACTIVE_SIM\": \"%s\",\n",dcu_st.active_sim);
    APPEND("        \"ISP\": \"%s\"\n",dcu_st.isp);
    APPEND("      },\n");
    APPEND("      \"REMOTE_ACCESS\": {\n");
    APPEND("        \"VPN_IP\": \"%s\",\n",dcu_st.vpn_ip);
    APPEND("        \"REMOTE_ACCESS_ENABLED\": \"%s\"\n",dcu_st.remote_access_enabled);
    APPEND("      }\n");
    APPEND("    },\n");
    /**************** METERS ****************/
    APPEND("    \"METERS\": [\n");
    {
        ...
        {
            const meter_entry_t *m = &meters[i];
            /* Print comma before every meter except the first */
            if (!first_meter)
            {
                APPEND(",\n");
            }
            ...
            APPEND("        \"BAY\": \"%s\",\n", m->bay);
            APPEND("        \"IP_ADDRESS\": \"%s\",\n", m->ip_address);
            APPEND("        \"COMM_STATUS\": \"%s\",\n", m->comm_status);
            APPEND("        \"COMM_INHIBIT\": \"%s\",\n", m->comm_inhibit);
            APPEND("        \"TIME_SYNCHED\": \"%s\",\n", m->time_synched);
            APPEND("        \"VPN_IP\": \"%s\",\n", m->vpn_ip);
            APPEND("        \"REMOTE_ACCESS_ENABLED\": \"%s\"\n", m->remote_access_enabled);
            APPEND("      }");
...
#undef APPEND
    *output_file_sz = written;
    LOG_INFO("build_health_status_json: JSON built (%zu bytes) output_file_sz=%d",written,*output_file_sz);
    printf("Output Buffer = %s\n", out_buf);
    rc = 0;
    return rc;
}
