 ...
 * ========================================================================= */
static int export_cmd_register(jbuf_t *, redisContext *,const char *, const char *,modbus_reg_t *, int);
static int export_cmd_modrtu_device(jbuf_t *, redisContext *, cmd_request_t *);
static int export_cmd_modtcp_device(jbuf_t *, redisContext *, cmd_request_t *);
...
int rget_str(redisContext *ctx, const char *key, const char *field,
...
int rget_int(redisContext *ctx, const char *key, const char *field,
                    int def)
{
    char buf[64];
...
int rhash_exists(redisContext *ctx, const char *key)
{
...
void export_dcu_nameplate(jbuf_t *jb, redisContext *ctx)
{
    ...
    /* Read DCU information from Redis */
    rget_str(ctx, key, "device",      devname,  sizeof(devname));
    rget_str(ctx, key, "serial_num",  serial,   sizeof(serial));
    rget_str(ctx, key, "dcu_uptime",  uptime,   sizeof(uptime));
    rget_str(ctx, key, "dcu_loc",     location, sizeof(location));
    ...
    jbuf_append(jb, "\"DEVNAME\":");
    jbuf_append_escaped(jb, devname);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"DEV_LOC\":");
    jbuf_append_escaped(jb, location);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"SN\":");
    jbuf_append_escaped(jb, serial);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"IMEI\":");
    jbuf_append_escaped(jb, imei);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"FW_VER\":");
    jbuf_append_escaped(jb, fw_ver);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"TS\":");
    jbuf_append_escaped(jb, datetime);
    jbuf_append(jb, "\n");
    ...
    jbuf_append(jb, "\"DCU_DETAILS\":{\n");

    jbuf_append(jb, "\"ATTRIBUTE_1\":");
    jbuf_append_escaped(jb, attr1);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"ATTRIBUTE_2\":");
    jbuf_append_escaped(jb, attr2);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"ATTRIBUTE_3\":");
    jbuf_append_escaped(jb, attr3);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"ATTRIBUTE_4\":");
    jbuf_append_escaped(jb, attr4);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"ATTRIBUTE_5\":");
    jbuf_append_escaped(jb,attr5);
    jbuf_append(jb, "\n");
...
static void export_register(jbuf_t *jb, redisContext *ctx,const char *reg_cfg_key,const char *value_hash, int is_last)
{
    char name[64];
    if (!rget_str(ctx, reg_cfg_key, "name_id", name, sizeof(name)))
        return;      /* register config doesn't exist */
    ...
    int is_coil = (func_type == 1 || func_type == 2);

    /* Register as array */
    jbuf_append(jb, "[");

    /* ADDRESS */
    jbuf_append(jb, "%d,", start_addr);

    /* NAME */
    jbuf_append_escaped(jb, name);
    jbuf_append(jb, ",");

    /* TYPE */
    jbuf_append(jb, "%d,", func_type);
...
{
    char cfg_key[128];
    snprintf(cfg_key, sizeof(cfg_key), "modrtu_serial%u_%u_cfg",port_id, dev_idx);
    ...
    /* Read status */
    char status_key[128];
    snprintf(status_key, sizeof(status_key),"modrtu_serial%u_%u_status",port_id, dev_idx);
    ...
    jbuf_append(jb, "\"NAME\": ");
    jbuf_append_escaped(jb, dev_name);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"STATUS\": ");
    jbuf_append_escaped(jb, status_out);
    jbuf_append(jb, ",\n");

    jbuf_append(jb, "\"LAST_UPDATE_TIME\": ");
    jbuf_append_escaped(jb, last_comm);
    jbuf_append(jb, ",\n");
    ...
    {
        char reg_cfg_key[128];
        snprintf(reg_cfg_key, sizeof(reg_cfg_key),"modrtu_serial%u_%u_reg_%d_cfg",port_id, dev_idx, r);
        char name_check[64];
        ...
        snprintf(value_hash,sizeof(value_hash),"modrtu:%u:%u:%d:%s",port_id,dev_idx,r,dev_name);
        reg_written++;
        ...
        {
            char peek_key[128];
            snprintf(peek_key,sizeof(peek_key),"modrtu_serial%u_%u_reg_%d_cfg",port_id,dev_idx,rr);
            char peek_name[64];
            if (rget_str(ctx,peek_key,"name_id",peek_name,sizeof(peek_name)))
            {
                int peek_fc = rget_int(ctx,peek_key,"func_type",3);
                                ...
                                uint16_t dev_idx, int is_last_device)
{
    ...
    for (int r = 0; r < max_regs; r++)
    {
        ...
        snprintf(reg_cfg_key,sizeof(reg_cfg_key),"modtcp_%u_reg_%d_cfg",dev_idx,r);
        ...
        snprintf(value_hash,sizeof(value_hash),"modtcp:eth:%u:%d:%s",dev_idx,r,dev_name);
        ...
        for (int rr = r + 1; rr < max_regs; rr++)
        {
            ...
            snprintf(peek_key,sizeof(peek_key),"modtcp_%u_reg_%d_cfg",dev_idx,rr);
            char peek_name[64];
            if (rget_str(ctx,peek_key,"name_id",peek_name,sizeof(peek_name)))
            {
                int peek_fc = rget_int(ctx,peek_key,"func_type",3);
...
//     jbuf_append(&jb, "\"TYPE\":\"CYCLIC_MESSAGE\",\n");
//     jbuf_append(&jb, "\"SEQ_NUM\":\"0004\",\n");
//     jbuf_append(&jb, "\"DATATYPE\":\"MODBUS_DATA_MESSAGE\",\n");
...
//     jbuf_append(&jb, "\"DATA\":{\n");
//     jbuf_append(&jb, "\"FIELDS\":[\"ADDRESS\",\"NAME\",\"TYPE\",\"VALUE\"],\n");
...
char *modbus_export_json(redisContext *ctx, uint16_t num_serial_ports)
{
    ...
    jbuf_append(&jb, "\"DATA\":{\n");
    jbuf_append(&jb,
                "\"FIELDS\":[\"ADDRESS\",\"NAME\",\"TYPE\",\"VALUE\"],\n");
    jbuf_append(&jb,
                "\"MODBUS_DEVICES\":[\n");

    /* ----------------------------------------------------------
     * Count enabled devices (RTU + TCP)
     * ---------------------------------------------------------- */
    ...
    /* TCP devices */
    for (uint16_t d = 0; d < MAX_TCP_DEVICES; d++)
    {
        char cfg_key[128];
        snprintf(cfg_key,sizeof(cfg_key),"modtcp_%u_cfg",d);
        if (rget_int(ctx,cfg_key,"enable_device",0) > 0)
...
char *modbus_cmd_export_json(redisContext *ctx, cmd_request_t *cmd)
{
    jbuf_t jb;
    ...
    int exported = 0;
    exported += export_cmd_modrtu_device(&jb, ctx, cmd);
    exported += export_cmd_modtcp_device(&jb, ctx, cmd);
    jbuf_append(&jb, "]");
    jbuf_append(&jb, "}");   /* DATA */
    jbuf_append(&jb, "}");   /* ROOT */
...
void export_cmd_dcu_nameplate(jbuf_t *jb, redisContext *ctx)
{
    char buf[128];
    jbuf_append(jb, "\"NP\":{");

    /* DEVNAME */
    if (rget_str(ctx, "dcu_info", "device", buf, sizeof(buf)))
    {
        jbuf_append(jb, "\"DEVNAME\":");
    ...
    if (rget_str(ctx, "dcu_info", "dcu_loc", buf, sizeof(buf)))
    {
        jbuf_append(jb, ",\"DEV_LOC\":");
        jbuf_append_escaped(jb, buf);
    }
    ...
    /* Serial Number */
    if (rget_str(ctx, "dcu_info", "serial_num", buf, sizeof(buf)))
    {
        jbuf_append(jb, ",\"SN\":");
        jbuf_append_escaped(jb, buf);
    }
    ...
    if (rget_str(ctx, "dcu_info", "fw_ver", buf, sizeof(buf)))
    {
        jbuf_append(jb, ",\"FW_VER\":");
        jbuf_append_escaped(jb, buf);
    }
...
static int export_cmd_register(jbuf_t *jb,redisContext *ctx,const char *reg_cfg_key,const char *value_hash,modbus_reg_t *req_reg, int is_last)
{
    char name[64];

    /* Register configuration must exist */
    if (!rget_str(ctx, reg_cfg_key, "name_id", name, sizeof(name)))
        return 0;

    int func_type  = rget_int(ctx, reg_cfg_key, "func_type", 3);
    int start_addr = rget_int(ctx, reg_cfg_key, "start_addr", -1);

    /* This isn't the requested register */
    if (start_addr != req_reg->address)
        return 0;
    ...
    snprintf(field, sizeof(field), "reg_%d", start_addr);

    if (!rget_str(ctx, value_hash, field, value, sizeof(value)))
        value[0] = '\0';
    ...
    */
    ...
    {
        jbuf_append_coil_value(jb, value);
    }
    else
    {
        jbuf_append_numeric_value(jb, value);
    }
...
static int export_cmd_modrtu_device(jbuf_t *jb,redisContext *ctx,cmd_request_t *cmd)
{
    int exported = 0;

    for (int dev_idx = 0; dev_idx < MAX_RTU_DEVICES; dev_idx++)
    {
        char cfg_key[128];

        snprintf(cfg_key,sizeof(cfg_key),"modrtu_serial1_%d_cfg",dev_idx);
        if (rget_int(ctx, cfg_key, "enable_device", 0) == 0)
            continue;
        int slave_id = rget_int(ctx, cfg_key, "slave_id", 0);
        if (slave_id != cmd->slave_id)
            continue;
        ...
        snprintf(status_key,sizeof(status_key),"modrtu_serial1_%d_status",dev_idx);

        char status[64];
        char last_comm[64];

        rget_str(ctx,status_key,"status",status,sizeof(status));

        rget_str(ctx,status_key,"last_communication",last_comm,sizeof(last_comm));

        const char *status_out = "Not Connected";

        if (!strcmp(status, "communicating"))
            status_out = "Connected";
        ...
        for (int r = 0; r < num_points; r++)
        {
            ...
            snprintf(reg_cfg_key,sizeof(reg_cfg_key),"modrtu_serial1_%d_reg_%d_cfg",dev_idx,r);
            char value_hash[128];
            snprintf(value_hash,sizeof(value_hash),"modrtu:1:%d:%d:%s",dev_idx,r,dev_name);

            for (int req = 0; req < cmd->reg_count; req++)
            {
                reg_written += export_cmd_register(jb,ctx,reg_cfg_key,value_hash,&cmd->regs[req],0);
            }
        }
...
static int export_cmd_modtcp_device(jbuf_t *jb,redisContext *ctx,cmd_request_t *cmd)
{
    int exported = 0;

    for (int dev_idx = 0; dev_idx < MAX_TCP_DEVICES; dev_idx++)
    {
        char cfg_key[128];
        ...
        jbuf_append(jb,
            "\"IP_ADDRESS\":");
        jbuf_append_escaped(jb, ip_addr);
        ...
        for (int r = 0; r < num_points; r++)
        {
            ...
            snprintf(reg_cfg_key,
                     sizeof(reg_cfg_key),
                     "modtcp_%d_reg_%d_cfg",
                     dev_idx,
                     r);
            ...
            for (int req = 0; req < cmd->reg_count; req++)
            {
                reg_written += export_cmd_register(
                                    jb,
                                    ctx,
                                    reg_cfg_key,
                                    value_hash,
