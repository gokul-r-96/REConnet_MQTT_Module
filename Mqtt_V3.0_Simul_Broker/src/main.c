...
//         }
//         /*No Broker Connected - Trying both primary and secondary broker with in the given amount of intervals*/
//         else
//         {
//             primary.connected = false;
//             secondary.connected = false;
...
void *mqtt_worker_thread(void *arg)
{
    ...
    while (stop_flag)
    {
        ...
        /* PRIMARY connection watchdog */
        if (primary_connecting &&
            primary_connect_start > 0 &&
            (now - primary_connect_start >= MQTT_CONNECT_TIMEOUT))
        {
            LOG_ERROR("[WATCHDOG] PRIMARY connection timeout");
            ...
            LOG_INFO("[WATCHDOG] PRIMARY connection released");
        }

        /* SECONDARY connection watchdog */
        if (secondary_connecting &&
            secondary_connect_start > 0 &&
            (now - secondary_connect_start >= MQTT_CONNECT_TIMEOUT))
        {
            LOG_ERROR("[WATCHDOG] SECONDARY connection timeout");
            ...
            LOG_INFO("[WATCHDOG] SECONDARY connection released");
        }
         ...
         * CONNECTION STATE
         *
         * Both brokers are independent.
         * There is NO preferred broker.
         * ========================================================= */
        ...
        /*
         * Keep connection states independently.
         *
         * current_active is retained only because the existing
         * command/Redis logic still uses it.
         *
         * It is NOT used to decide MQTT connections.
         */
        ...
        /*
         * Set current_active only for existing code which requires
         * one connection pointer.
         *
         * This does NOT disconnect the other broker.
         *
         * PRIMARY is selected only as a compatibility reference
         * when both are connected.
         */

        if (primary.connected)
        {
            current_active_mqtt1 = &primary;

            cur_active_mqtt = (certificate_path_check_primary == 0) ? 1 : 2;
        }
        else if (secondary.connected)
        {
            current_active_mqtt2 = &secondary;

            cur_active_mqtt = (certificate_path_check_secondary == 0) ? 1 : 2;
        }
        else
        {
            current_active_mqtt1 = NULL;
            current_active_mqtt2 = NULL;
            cur_active_mqtt = -1;
        }
         ...
         * PRIMARY CONNECTION
         *
         * PRIMARY has its OWN retry timer.
         * It does not depend on SECONDARY.
         * ========================================================= */

        if (primary.cfg.enable_mqtt && !primary.connected && !primary_connecting && (now - last_primary_try >= PRIMARY_RETRY_SEC))
        {
            LOG_INFO("[RECONNECT] Trying PRIMARY");
         ...
         *
         * SECONDARY has its OWN retry timer.
         * It does not depend on PRIMARY.
         * ========================================================= */

        if (secondary.cfg.enable_mqtt && !secondary.connected && !secondary_connecting && (now - last_secondary_try >= SECONDARY_RETRY_SEC))
        {
            LOG_INFO("[RECONNECT] Trying SECONDARY");
        ...
        /* =========================================================
         * DESTROY OLD PRIMARY CLIENT
         * ========================================================= */

        if (primary_need_destroy &&
            (monotonic_sec() - primary_destroy_time) >= 2)
        {
            if (primary.client)
            {
                LOG_INFO("[PRIMARY] Destroying old client");
        ...
        if (check_redis_resp == 1)
            read_redis_resp(current_active_mqtt1);


        /* =========================================================
         * INSTANTANEOUS DATA
         *
         * SEND TO BOTH CONNECTED BROKERS
         * ========================================================= */

        if ((primary.connected || secondary.connected) && (now - last_publish_inst >= current_active_mqtt1->cfg.dlms_inst_pub_interval * 60))
        {
            last_publish_inst = now;
            load_active_meters(ctx);
            for (int i = 0; i < meter_count; i++)
            {
                ...
                if (res.status == 0)
                {
                    if (primary.connected)
                    {
                        LOG_INFO("[PUBLISH] Instantaneous data -> MQTT BROKER 1");
                        mqtt_send_file(&primary,res.filename,INST_DATA_TOPIC);
                    }
                    if (secondary.connected)
                    {
                        LOG_INFO("[PUBLISH] Instantaneous data -> MQTT BROKER 2");
                        mqtt_send_file(&secondary,res.filename,INST_DATA_TOPIC);
                    }
        ...
        if (check_redis_resp == 1)
            read_redis_resp(current_active_mqtt2);
        ...
        if ((primary.connected) && (now - last_publish_profile >= current_active_mqtt1->cfg.dlms_data_pub_interval * 60))
        {
            last_publish_profile = now;
            time_t t = time(NULL);
            struct tm *tm_det = localtime(&t);
            ...
            for (int i = 0; i < meter_count; i++)
            {
                ...
                LOG_INFO("Meter %s - Time taken for file generation alone : %ld ms (%.3f seconds)",serial,elapsed_ms,elapsed_ms / 1000.0);
                ...
                if (res.status == 0)
                {
                    ...
                    if (primary.connected)
                    {
                        LOG_INFO("[PUBLISH] Meter profile -> MQTT BROKER 1");
                        mqtt_send_file(&primary,res.filename,METER_DATA_TOPIC);
                    }
                    if (secondary.connected)
                    {
                        LOG_INFO("[PUBLISH] Meter profile -> MQTT BROKER 2");
                        mqtt_send_file(&secondary,res.filename,METER_DATA_TOPIC);
                    }
                    memset(file_rem_cmd,0,sizeof(file_rem_cmd));
                    sprintf(file_rem_cmd,"rm %s",res.filename);
                    system(file_rem_cmd);
        ...
        if ((primary.connected || secondary.connected) && (now - last_publish_hc >= current_active_mqtt1->cfg.hc_pub_interval * 60))
        {
            load_active_meters(ctx);
            last_publish_hc = monotonic_sec();
            char xml_buf[PAYLOAD_BUFFER_SIZE];
            ...
            if (primary.connected)
            {
                LOG_INFO("[PUBLISH] Health data -> PRIMARY");
                mqtt_send_msg(&primary,xml_buf,file_Size,HEALTH_DATA_TOPIC);
            }

            if (secondary.connected)
            {
                LOG_INFO("[PUBLISH] Health data -> SECONDARY");
                mqtt_send_msg(&secondary,xml_buf,file_Size,HEALTH_DATA_TOPIC);
            }
        }
        ...
        if ((primary.connected || secondary.connected) && (now - last_publish_modbus >= current_active->cfg.modbus_data_pub_interval * 60))
        {
            last_publish_modbus = now;
            char *json = modbus_export_json(ctx, 2);
            if (json)
            {
                ...
                if (primary.connected)
                {
                    LOG_INFO("[PUBLISH] Modbus data -> PRIMARY");
                    mqtt_send_msg(&primary,json,strlen(json),MODBUS_DATA_TOPIC);
                }

                if (secondary.connected)
                {
                    LOG_INFO("[PUBLISH] Modbus data -> SECONDARY");
                    mqtt_send_msg(&secondary,json,strlen(json),MODBUS_DATA_TOPIC);
                }
                free(json);
            }
            else
        ...
        /* =========================================================
         * PUBLISHING TIME INFORMATION
         * ========================================================= */

        now = monotonic_sec();

        if (current_active_mqtt1)
        {
            LOG_INFO("--------------------------------------PUBLISHING INTERVALS FOR MQTT BROKER 1 BEGINS--------------------------------------------");
            interval_sec = current_active_mqtt1->cfg.modbus_data_pub_interval * 60;
            elapsed = now - last_publish_modbus;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
                LOG_INFO("Modbus messages will publish in %d min %02d sec",remaining / 60,remaining % 60);
            }

            interval_sec = current_active_mqtt1->cfg.hc_pub_interval * 60;
            elapsed = now - last_publish_hc;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
                LOG_INFO("Health check messages will publish in %d min %02d sec",remaining / 60,remaining % 60);
            }

            interval_sec = current_active_mqtt1->cfg.dlms_data_pub_interval * 60;
            elapsed = now - last_publish_profile;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
                LOG_INFO("Meter Profile Data will publish in %d min %02d sec",remaining / 60,remaining % 60);
            }

            interval_sec = current_active_mqtt1->cfg.dlms_inst_pub_interval * 60;
            elapsed = now - last_publish_inst;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
                LOG_INFO("Instantaneous Data will publish in %d min %02d sec",remaining / 60,remaining % 60);
            }
            LOG_INFO("------------------------------------PUBLISHING INTERVALS FOR MQTT BROKER 1 ENDS------------------------------------------------");
        }


        if (current_active_mqtt2)
        {
            LOG_INFO("--------------------------------------PUBLISHING INTERVALS FOR MQTT BROKER 2 BEGINS--------------------------------------------");
            interval_sec = current_active_mqtt2->cfg.modbus_data_pub_interval * 60;
            elapsed = now - last_publish_modbus;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
            ...
            interval_sec = current_active_mqtt2->cfg.hc_pub_interval * 60;
            elapsed = now - last_publish_hc;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
            ...
            interval_sec = current_active_mqtt2->cfg.dlms_data_pub_interval * 60;
            elapsed = now - last_publish_profile;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
            ...
            interval_sec = current_active_mqtt2->cfg.dlms_inst_pub_interval * 60;
            elapsed = now - last_publish_inst;
            remaining = interval_sec - elapsed;
            if (remaining > 0)
            {
            ...
            LOG_INFO("------------------------------------PUBLISHING INTERVALS FOR MQTT BROKER 2 ENDS------------------------------------------------");
        }
        ...
        if (mqtt_status_update_req)
        {
            ...
            if (strcmp(mqtt_status_value, "connected") == 0)
            {
                 ...
                 * Both brokers are independent.
                 *
                 * Update each broker according to its
        ...
        if (mqtt_time_update_req)
        {
            ...
            if (mqtt_time_active == 0)
            {
                ...
                /*
                 * Do not force secondary uptime to 0 if it is
                 * actually connected.
                 */
                if (!secondary.connected)
                {
            ...
            else if (mqtt_time_active == 1)
            {
                ...
                /*
                 * Do not force primary uptime to 0 if it is
                 * actually connected.
                 */
                if (!primary.connected)
                {
         ...
         * MQTT COMMAND
         *
         * Existing command logic retained.
         * ========================================================= */
        ...
        if (mqtt_cmd_recv)
        {
            ...
            mqtt_cmd_recv = 0;
            pthread_mutex_unlock(&cmd_mutex);
            processServerMsg(current_active,
                             local_cmd);
        }
