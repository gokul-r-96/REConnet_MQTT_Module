...
CDFInstMapping cdf_inst_mappings[] = {
    ...
    // rithika 15Apr2026 commented because in default only the subset is needed 
...
int create_redis_hash(redisContext *c, const char *hash_name, ObisMapping *mappings, int count) {
    ...
    for (int i = 0; i < count; i++) {
      if(mappings[i].param_name[0] != '\0'){    // rithika 15Apr2026 added to create the cdf hash only for param name in the list (cdf_inst_mappings, and for other datatype list also)
...
int process_data_type(redisContext *c, DataType data_type, int use_cdf_format) {
    ...
    // Create JSON file
    // rithika 15Apr2026 commented because o use of this json map.
